...
from qmf.utils.metrics import psnr, ssim, msssim, bits_per_pixel, compression_ratio
...
def eval_compression(
    ...
    **kwargs,
    ...
    psnr_value = psnr(reconstructed, image).item()
    ssim_value = ssim(reconstructed, image).item()
    msssim_value = msssim(reconstructed, image).item()
...
class Plot:
    ...
    def interpolate(
        ...
        for c, grp in self.data.groupby(groupby):
            ...
            loess = LOESS(frac=np.arange(0.15, 0.75, 0.1), degree=1)
